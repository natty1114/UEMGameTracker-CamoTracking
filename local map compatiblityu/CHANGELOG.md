...
### Added
- Apps Script submission endpoint support. When `submission_endpoint_url` is configured in `config/uem_config.json`, reports are sent to the web endpoint instead of using local Google service-account credentials.
- `submission_endpoint_secret` and per-install `submission_client_id` config values for endpoint authentication and rate limiting.
- `apps_script_submission_endpoint.gs`, a ready-to-deploy Google Apps Script web app that validates Steam Workshop links, checks a shared secret, rate-limits clients, rate-limits repeated Steam map submissions, writes copyable rows to `Submissions`, and writes audit details to `Submission Metadata`.
- Settings Project State now shows whether the submission endpoint and secret are configured, and whether the app is using the endpoint or local credential fallback.
- Local PyInstaller packaging script and spec for the map compatibility app: `build_map_compat.py` and `UEMMapCompatibility.spec`.

### Fixed
- Help and Settings windows now open from the packaged map compatibility app process instead of trying to launch sibling Python scripts with `sys.executable`.
- Packaged map compatibility builds no longer require `help_page.py` and `settings_page.py` to be launched as separate scripts.

### Changed
- Public map compatibility releases skip Google service-account credential JSON files by default.
- Submission writing now prefers the Apps Script endpoint and only falls back to `config/google_credentials.json` when no endpoint URL is configured.
- Map compatibility release output now includes `UEMMapCompatibility.exe`, runtime assets, config, and a `scripts/` source snapshot.

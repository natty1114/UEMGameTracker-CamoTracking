...
def _get_cache_dir():
    ...
    cache_path = get_runtime_dir(CACHE_DIR_NAME)
...
def get_workshop_image_url(steam_link_id):
    """Return the Steam-hosted preview image URL for Discord/external consumers."""
    if not steam_link_id or str(steam_link_id).strip() == "0":
        return None
    return _scrape_image_url(str(steam_link_id))
